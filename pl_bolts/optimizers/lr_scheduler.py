...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self) -> List[float]:  # type: ignore[override]
        ...
        return [
            (
                1 + math.cos(
                    math.pi * (self.last_epoch - self.warmup_epochs) /  # type: ignore[attr-defined]
                    (self.max_epochs - self.warmup_epochs)
                )
            ) /
            (
                1 + math.cos(
                    math.pi * (self.last_epoch - self.warmup_epochs - 1) /  # type: ignore[attr-defined]
                    (self.max_epochs - self.warmup_epochs)
                )
            ) * (group["lr"] - self.eta_min) + self.eta_min
            for group in self.optimizer.param_groups  # type: ignore[attr-defined]
        ]
